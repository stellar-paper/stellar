...
def generate_critical_regions(population, 
    ...
    X = [v[0].vars for v in population.get("X")]
    F, CB, SO = population.get("F", "CB", "SO")
    ...
    if save_folder is not None:
        ...
        with open(save_bounds_regions, 'w', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
            header = ['region', 'variable', 'lower_bound', 'upper_bound']
            writer.writerow(header)
            for i, region in enumerate(critical_regions):
                for var_name, lb, ub in zip(feature_names, region.xl, region.xu):
                    writer.writerow([f"region {i}", var_name, lb, ub])
    return regions
