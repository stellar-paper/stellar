...
import opensbt.algorithm.classification.decision_tree.decision_tree_llm as decision_tree
...
def write_summary_results(res, save_folder, params):
    ...
    from llm.llms import (
        ...
        call_counter_local,
        end_to_end_time,
        end_to_end_time_local,
        token_usage,
        token_usage_local,
    )

    # cost
    approx_costs = token_usage * 0.0015 / 1000

    approx_costs_local = token_usage_local * 0.0015 / 1000
    ...
    with open(
        ...
        write_to.writerow(["Tokens Used", token_usage])
        write_to.writerow(["LLM Calls", call_counter])
        write_to.writerow(["Costs Approximated", approx_costs])

        write_to.writerow(["Local Tokens Used", token_usage_local])
        write_to.writerow(["Local LLM Calls", call_counter_local])
        write_to.writerow(["Local Costs Approximated", approx_costs_local])

        write_to.writerow(
            [
                "Average Call Time",
                end_to_end_time / call_counter if call_counter > 0 else 0,
            ]
        )
        write_to.writerow(
            [
                "Average Call Time Local",
                end_to_end_time_local / call_counter_local
                if call_counter_local > 0
                else 0,
            ]
        )
    ...
    log.info(["Costs Approximated", approx_costs])
    log.info(["Local Tokens Used", token_usage_local])
    log.info(["Local Costs Approximated", approx_costs_local])
...
def design_space(
    ...
    problem = res.problem
    design_names = problem.names_dim_utterance
    n_var = len(problem.names_dim_utterance)
    xl = [0] * len(problem.names_dim_utterance)
    xu = [1] * len(problem.names_dim_utterance)
        ...
        for axis_y in range(axis_x + 1, n_var):
            ...
            if classification_type == ClassificationType.DT:
                ...
                if len(not_critical) != 0:
                    vars_list = [v.ordinal_vars for v in not_critical.get("X")[:, 0]]
                    X = np.array([v[axis_x] for v in vars_list])
                    Y = np.array([v[axis_y] for v in vars_list])
                    ax.scatter(
                        X,
                        Y,
                        s=40,
                ...
                if len(critical) != 0:
                    vars_list = [v.ordinal_vars for v in critical.get("X")[:, 0]]
                    X = np.array([v[axis_x] for v in vars_list])
                    Y = np.array([v[axis_y] for v in vars_list])
                    ax.scatter(
                        X,
                ...
                if len(critical_opt) != 0:
                    vars_list = [v.ordinal_vars for v in critical_opt.get("X")[:, 0]]
                    X = np.array([v[axis_x] for v in vars_list])
                    Y = np.array([v[axis_y] for v in vars_list])
                    ax.scatter(
                        X,
                ...
                if len(not_critical_opt) != 0:
                    vars_list = [
                        v.ordinal_vars for v in not_critical_opt.get("X")[:, 0]
                    ]
                    X = np.array([v[axis_x] for v in vars_list])
                    Y = np.array([v[axis_y] for v in vars_list])
                    ax.scatter(
...
@weave.op()
def all_individuals_llm(res, save_folder):
    ...
    with open(
        save_folder + "all_testcases_llm.csv", "w", encoding="UTF8", newline=""
    ) as f:
        write_to = csv.writer(f)
        ...
        # check if seeded TODO need find another way
        is_seeded = hasattr(all_individuals[0].get("X")[0], "seed")
        if is_seeded:
            header.append("seed")
        ...
        for index, ind in enumerate(all_individuals):
            ...
            if is_seeded:
                row.extend([f"%s" % X_i.seed for X_i in ind.get("X")])
            row.extend([f"%s" % X_i.question for X_i in ind.get("X")])
            row.extend([f"%s" % X_i.answer for X_i in ind.get("X")])
        ...
        with open(
            ...
            for i in range(len(algo.pop)):
                ...
                if isinstance(algo.pop.get("X")[i][0], Utterance):
                    row.extend([f"%s" % X_i.question for X_i in algo.pop.get("X")[i]])
                    row.extend([f"%s" % F_i for F_i in algo.pop.get("F")[i]])
                else:
                    row.extend(
